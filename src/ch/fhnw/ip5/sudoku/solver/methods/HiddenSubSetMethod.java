...
//		b.cluesPrint();
		...
		for (byte subsetsize = 2; subsetsize < size; subsetsize++) {
			...
			for (byte i = 0; i < size; i++) {
				if (checkContainer(rows[i],size, subsetsize)) { return true; }
				if (checkContainer(columns[i], size, subsetsize)) { return true; }
				if (checkContainer(boxes[i], size, subsetsize)) { return true; }
			}
...
//		}
	...
	private boolean checkRecursive(Set<Cell>[] pencilsets, byte subsetsize, int pos, int count, HashSet<Cell> solset, boolean[] subsetvalues) {
		...
		if (pos >= size || count > subsetsize) return false;
...
//					if (subsetvalues[i]) System.out.println("Subsetvalue: " + (i+1));;
//				}
//				
//				for (Cell c : tempSet) {
//					System.out.println("Cell #" + c.getHpos() + " " + c.getWpos());
...
//				System.out.println();
				...
				if (tempSet.size() == subsetsize && count == subsetsize) {
					...
					if (checkRecursive(pencilsets, subsetsize, k+1, count+1, tempSet, subsetvalues)) return true;
